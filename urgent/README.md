...
A three-tier audio attention system for [Claude Code](https://claude.com/claude-code), plus the `urgent` skill that fires the loudest tier when Claude genuinely needs you.
...
| Loud | Submarine (3x, 1.333s pauses) | The `urgent` skill. Claude decides this needs you NOW. | 0-1 |

The `urgent` skill itself is the headline piece. The supporting scripts and hooks make it usable in a real setup where concurrent sessions and background `claude -p` workers would otherwise drown the signal.

## What you get

- **`SKILL.md`**: the `urgent` skill. Claude fires it autonomously when forward progress is blocked on you AND consequences exist if you do not see it soon. Also fires on `/urgent` for manual testing.
...
- **`scripts/urgent.sh`**: triple Submarine with 1.333s pauses. The skill calls this.
...
If you have any `claude -p` workers running (scheduled prompts, cron jobs, queue workers, automated pipelines), each one fires the Stop hook on completion. With several workers running every few seconds you can hit a thousand-plus chimes per hour. The headless-suppression check in `play.sh` and `afplay-shadow.sh` looks at the parent process command and silently drops audio (but logs the event) when the parent matches `claude -p` or `claude --print`. Interactive sessions still chime normally.

## Install

```bash
# Copy the skill into your global skills dir
cp -r urgent ~/.claude/skills/

# Make the scripts executable
chmod +x ~/.claude/skills/urgent/scripts/*.sh

# Smoke test the loud tier
~/.claude/skills/urgent/scripts/urgent.sh "install test"
```
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/skills/urgent/scripts/play.sh Tink"
          }
        ]
      }
    ],
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/skills/urgent/scripts/play.sh Sosumi"
          }
        ]
      }
    ]
...
Swap any sound name in the hook commands or in `urgent.sh`. Test before committing:
...
```bash
grep "URGENT triple-ping" ~/.claude/logs/audio-hook.log
```
